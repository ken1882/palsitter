...
@dataclass(frozen=True)
class ExecutableFirewallStatus:
    supported: bool
    path: str
    allowed: bool = False
    blocked: bool = False
...
def _fix_port_script(
    ...
    return (
        ...
        f"{remove_block}{_replace_rule_block(rule_name)} "
        "New-NetFirewallRule "
...
class FirewallService:
    ...
    def check_port(
        ...
        *,
        ...
        """Check an inbound port using the supported platform firewall backend."""
        port = int(port)
        protocol = protocol.casefold()
    ...
    def check_executable(
        ...
        *,
        ...
        del root_password
        executable = str(Path(path).resolve(strict=False))
        if not self.supported or self.backend in _LINUX_BACKENDS:
            return ExecutableFirewallStatus(False, executable)
        try:
            if self._test_state_path:
                if self._test_delay > 0:
                    time.sleep(self._test_delay)
                state = Path(self._test_state_path).read_text(
                    encoding="utf-8"
                ).strip().casefold()
                block_name = state.partition(":")[2].strip()
                return ExecutableFirewallStatus(
                    True,
                    executable,
                    allowed=state == "open",
                    blocked=bool(block_name),
                    block_rule_names=(block_name,) if block_name else (),
                )
            try:
                result = self.run_command(
                    [
            ...
            for rule in rules:
                if not _matches_program(rule, executable):
                    continue
                if _rule_is_enabled_allow(rule):
                    allowed = True
                elif _rule_is_enabled_block(rule):
                    blocked = True
                    block_names.append(
                        str(rule.get("Name") or "(unnamed rule)")
                    )
            return ExecutableFirewallStatus(
                True,
                executable,
    ...
    def ensure_port(
        ...
        *,
        ...
        executable_status = (
            self.check_executable(executable, root_password=root_password)
            if executable is not None
            else None
        )
        if executable_status is not None and executable_status.error:
            raise FirewallError(executable_status.error)
        if (
            status.allowed
            and not status.blocked
            and not (executable_status and executable_status.blocked)
        ):
            return
        self._run_fix_payload(
            self._ensure_port_payload(status, executable_status),
            root_password,
        )
    ...
    def _ensure_port_payload(
        ...
        executable_status: ExecutableFirewallStatus | None,
    ) -> dict[str, Any]:
        protocol = status.protocol.casefold()
        executable_block_names = (
            executable_status.block_rule_names if executable_status else ()
        )
        return {
            "kind": "port",
            ...
            "port": status.port,
            "protocol": protocol,
            "rule_name": f"Palsitter-{protocol.upper()}-{status.port}",
            "display_name": f"Palsitter {protocol.upper()} {status.port}",
            "remove_names": list(
                dict.fromkeys(
                    [
    ...
    def _run_elevated(
        ...
        try:
            ...
            script = (
                ...
                f"-ArgumentList @('-m','module.firewall','--elevated-fix',{_quote_powershell(encoded)}) "
...
def check_port(port: int, protocol: str) -> PortFirewallStatus:
    return FirewallService().check_port(port, protocol=protocol)


def check_executable(path: str | Path) -> ExecutableFirewallStatus:
    return FirewallService().check_executable(path)
...
def ensure_port(
    ...
    FirewallService().ensure_port(port, protocol, executable)
