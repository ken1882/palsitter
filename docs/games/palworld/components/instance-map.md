...
displays Fast Travel and Watchtower markers.
...
- Online players come from the shared `PalRestCache`; the page does not issue its
  own REST requests. Each valid player position is an 8px-radius red dot.
...
navigation path, map switching, selected-map player filtering, overlays, player
selection, zoom/drag behavior, responsive layout, and cache-only player updates.
